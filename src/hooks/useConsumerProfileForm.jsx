...
export const useConsumerProfileForm = ({
  ...
  const saveSingleField = async (key) => {
    ...
    try {
      ...
      const { data } = await axios.put(
        `${apiRoot}${ApiPaths.consumer.updateProfile}`,
        formData,
        {
      ...
      const nextProfile = data?.user || data?.consumer || { ...(userProfile || {}) };
  ...
  const onPickImage = async (event) => {
    ...
    try {
      ...
      const { data } = await axios.put(
        `${apiRoot}${ApiPaths.consumer.profileImage}`,
        fd,
        {
          headers: { ...authHeaders, "Content-Type": "multipart/form-data" },
        }
      ...
      const nextProfile = data?.user || data?.consumer || { ...(userProfile || {}) };
      if (data?.user?.image || data?.consumer?.image || data?.image) {
        nextProfile.image = data?.user?.image || data?.consumer?.image || data.image;
      }
