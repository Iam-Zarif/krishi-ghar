...
export default function useResellerApprovedProducts({
  ...
  const handleSell = async (product, sellPostPayload = null) => {
    ...
    try {
      ...
      if (res?.sellPost?._id) {
        const nextRemainingQty = Number(res?.stock?.newRemainingQty);
        setProducts((prev) =>
          prev.map((item) =>
            item._id === product._id
              ? {
                  ...item,
                  quantity: Number.isFinite(nextRemainingQty)
                    ? Math.max(0, nextRemainingQty)
                    : item.quantity,
                  isSelling: Number.isFinite(nextRemainingQty)
                    ? nextRemainingQty <= 0
                    : true,
                }
              : item
