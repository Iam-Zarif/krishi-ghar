...
export default function ConsumerWishlist() {
  ...
  const filtered = useMemo(() => {
    ...
    if (q) {
      ...
      arr = arr.filter((i) => {
        const hay = [i.productName, i.description, i.categoryName]
          .join(" ")
          .toLowerCase();
  ...
  return (
    <div className="w-full px-4 pt-6 pb-24 max-w-7xl lg:pt-[6rem] mx-auto">
      ...
      {error && (
        <ErrorBlock message={error} onRetry={() => setReloadTick((t) => t + 1)} />
      )}
              ...
              <div key={it.id} className="h-full">
                <div className="bg-white border border-gray-200 rounded-xl h-full flex flex-col">
                  ...
                  <div className="p-4 bg-gray-50 rounded-b-xl flex-1 flex flex-col min-w-0">
                    ...
                    <div className="mt-1 text-xs text-gray-400">যোগ হয়েছে {added}</div>
                    <button
                      type="button"
